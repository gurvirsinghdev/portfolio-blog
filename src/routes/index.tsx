...
const getPosts = createServerFn({ method: "GET" })
  .handler(() => db.select().from(postsTable).orderBy(desc(postsTable.createdAt)));
            ...
            <Link key={idx} to={'/$postId'} params={{ postId: post.id }}>
              <div className='space-y-3 group border-2'>
                <div className='bg-muted w-full' style={{
                  ...
                  <img className='w-full h-full object-cover' src={`https://picsum.photos/seed/${post.title.replaceAll(' ', '')}/1280/720?${idx}`} alt='Blog post cover' />
                </div>
                <div className='space-y-1 p-3'>
                  <h2 className='text-xl capitalize group-hover:underline line-clamp-2 font-semibold text-foreground leading-snug'>{post.title}</h2>
